#!/usr/bin/env bash
# Deploy Poof to Stellar testnet. Permissionless: friendbot funds the deployer,
...
cargo build -p poof-contract --target wasm32v1-none --release
WASM=target/wasm32v1-none/release/poof_contract.wasm
stellar contract optimize --wasm "$WASM" 2>/dev/null || true
OPT=target/wasm32v1-none/release/poof_contract.optimized.wasm
[ -f "$OPT" ] && WASM="$OPT"
...
# Native XLM Stellar Asset Contract (testnet). Override with POOF_TOKEN.
TOKEN="${POOF_TOKEN:-$(stellar contract id asset --asset native --network "$NETWORK")}"

...
//! from the circuits component. This module does not require them to be present at
...
pub enum SnarkjsInvocation {
    /// `node <path-to-snarkjs/cli.js>` for a local snarkjs install.
...
mod tests {
    ...
    fn prove_without_artifacts_errors_not_panics() {
        ...
        let r = prove("{}", &cfg, Path::new("/tmp/poof-prove-test"));
        assert!(r.is_err());
    }
    ...
    #[test]
    #[ignore = "requires circuit artifacts from the circuits component"]

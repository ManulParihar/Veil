...
//!
//! Two distinct families derive from the one 32-byte seed (CLAUDE.md Part 8):
//!
//! * The **BN254 spend / viewing keys** — `sk`, `nk`, `ivk`, `ovk` — come from
//!   `veil-crypto::Seed` (Poseidon over the field). These live in-circuit.
...
//! is ever mixed into a key path (CLAUDE.md client edge case: "Deterministic key
//! derivation must be reproducible across devices from the seed alone").

use hkdf::Hkdf;
use sha2::Sha256;
use veil_crypto::{fr_to_be_bytes, Keypair, Seed};
...
impl Keys {
    ...
    /// Nullifier key `nk` (reserved for Phase-2 nk/ak split).

...
//!
//! This is NOT a reimplementation — it delegates to `veil-crypto`, the single
//! source of Poseidon truth (INTERFACES §0). The contract consumes `veil-crypto`
...
//! elements); `veil-crypto` speaks `ark_bn254::Fr`. We convert at this seam.

use soroban_sdk::{BytesN, Env};
use veil_crypto::{compress as fr_compress, fr_from_be_bytes, fr_to_be_bytes, zero_leaf};

/// `Poseidon(left, right)` — the Merkle parent hash. Matches `veil-crypto::compress`.
